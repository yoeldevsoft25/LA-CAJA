-- Script de Utilidad: Crear Tablas Contables Faltantes
-- Uso: cat fix_missing_accounting_tables.sql | docker exec -i la-caja-db psql -U postgres -d la_caja
...
COMMENT ON TABLE accounting_budgets IS 'Presupuestos contables y financieros';
COMMENT ON TABLE accounting_budget_lines IS 'Partidas específicas de cada presupuesto';
COMMENT ON TABLE bank_statements IS 'Estados de cuenta bancarios para conciliación';
COMMENT ON TABLE bank_transactions IS 'Transacciones individuales de los estados de cuenta';
COMMENT ON TABLE accounting_audit_logs IS 'Logs de auditoría específicos para cambios contables';
