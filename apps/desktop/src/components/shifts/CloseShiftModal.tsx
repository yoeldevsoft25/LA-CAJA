...
const closeShiftSchema = z.object({
  ...
  counted_pago_movil_bs: z.number().min(0).optional(),
  counted_transfer_bs: z.number().min(0).optional(),
  counted_other_bs: z.number().min(0).optional(),
...
interface CloseShiftModalProps {
  ...
  onConfirm: (data: CloseShiftRequest) => void
  isLoading: boolean
}
...
export default function CloseShiftModal({
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<CloseShiftRequest>({
    resolver: zodResolver(closeShiftSchema),
    defaultValues: {
      counted_bs: expectedTotals ? Number(expectedTotals.cash_bs) || 0 : 0,
      counted_usd: expectedTotals ? Number(expectedTotals.cash_usd) || 0 : 0,
      counted_pago_movil_bs: expectedTotals ? Number(expectedTotals.pago_movil_bs) || 0 : undefined,
      counted_transfer_bs: expectedTotals ? Number(expectedTotals.transfer_bs) || 0 : undefined,
      counted_other_bs: expectedTotals ? Number(expectedTotals.other_bs) || 0 : undefined,
      note: '',
    },
  })
  ...
  const expectedBs = expectedTotals ? Number(expectedTotals.cash_bs) || 0 : 0
  const expectedUsd = expectedTotals ? Number(expectedTotals.cash_usd) || 0 : 0
  ...
  const onSubmit = (data: CloseShiftRequest) => {
    if (confirmStep === 1) {
      ...
      const roundedData = {
        ...
        counted_pago_movil_bs: data.counted_pago_movil_bs
          ? Math.round(data.counted_pago_movil_bs * 100) / 100
          : undefined,
        counted_transfer_bs: data.counted_transfer_bs
          ? Math.round(data.counted_transfer_bs * 100) / 100
          : undefined,
        counted_other_bs: data.counted_other_bs
          ? Math.round(data.counted_other_bs * 100) / 100
          : undefined,
      }
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Apertura: {Number(shift.opening_amount_bs).toFixed(2)} Bs
                          </p>
                        </div>
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Apertura: ${Number(shift.opening_amount_usd).toFixed(2)}
                          </p>
                        </div>
                      </div>
                    </CardContent>
              ...
              <div className="space-y-4 sm:space-y-6">
                <Alert className="bg-info/10 border-info/50">
                  ...
                  <AlertDescription>
                    ...
                    <p className="text-sm text-foreground mb-4">
                      Estás a punto de cerrar el turno. Este proceso es{' '}
