...
export class QuotaGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (requiredFeature) {
            const hasFeature = await this.licenseService.checkFeature(storeId, requiredFeature);
            if (!hasFeature) {
        ...
        const requiredQuota = this.reflector.getAllAndOverride<string>('required_quota', [
            context.getHandler(),
            context.getClass(),
        ]);
