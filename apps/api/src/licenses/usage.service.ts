...
import { LicenseUsage, StoreLicense, SubscriptionPlan } from '../database/entities';
...
export class UsageService {
    ...
    async checkQuota(storeId: string, metric: string) {
        ...
        const limit = license?.custom_limits?.[metric]
            ?? license?.plan?.limits?.[metric]
            ?? Infinity;

        if (currentUsed >= limit) {
            this.logger.warn(`Store ${storeId} exceeded quota for ${metric}: ${currentUsed}/${limit}`);
