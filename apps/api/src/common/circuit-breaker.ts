...
export class CircuitBreaker {
    ...
    async execute<T>(fn: () => Promise<T>): Promise<T> {
        ...
        if (currentState === 'OPEN') {
            ...
            throw new Error(`CircuitBreaker is OPEN based to failures. Retrying in ${waitSeconds}s.`);
        }
    ...
    private onSuccess() {
        if (this.status === 'HALF_OPEN') {
            this.successes++;
            this.logger.log(`CircuitBreaker request SUCCESS (${this.successes}/${this.config.successThreshold})`);
    ...
    private onFailure(error: any) {
        if (this.status === 'CLOSED') {
            this.failures++;
            this.logger.warn(`CircuitBreaker failure (${this.failures}/${this.config.failureThreshold}): ${error.message}`);
    ...
    private transitionTo(newState: CircuitBreakerStatus) {
        ...
        this.logger.warn(`CircuitBreaker state changed: ${oldState} -> ${newState}`);
