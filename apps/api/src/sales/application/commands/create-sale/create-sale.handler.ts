        ...
        if (
          ...
          try {
            ...
            if (consumedNumber) {
              ...
              this.logger.log(
                `[Phase 3] Fiscal number consumed for online sale: ${fiscalNumber}`,
              );
            }
          } catch (error) {
            this.logger.warn(
              `No se pudo consumir número fiscal preventivo: ${error instanceof Error ? error.message : String(error)}`,
            );
          }
        }
