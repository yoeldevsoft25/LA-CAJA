...
export class RealTimeAnalyticsService {
  ...
  async handleInventoryMetrics() {
    this.logger.log(
      'Iniciando cálculo de métricas de inventario (cada 2 horas)...',
    );
  ...
  async handleDebtMetrics() {
    this.logger.log(
      'Iniciando cálculo de métricas de deudas (cada 6 horas)...',
    );
  ...
  private async processStoresMetrics(
    category: 'sales' | 'inventory' | 'debt' | 'daily',
  ) {
    try {
      ...
      for (const store of stores) {
        try {
          switch (category) {
            case 'sales':
              await this.calculateSalesMetrics(store.id);
              break;
            case 'inventory':
              await this.calculateInventoryMetrics(store.id);
              break;
            case 'debt':
              await this.calculateCustomerDebtMetrics(store.id);
              break;
            case 'daily':
              await this.calculateDailyAggregates(store.id);
              break;
          }
          await this.checkThresholds(store.id);
        } catch (error) {
          this.logger.error(
            `Error procesando ${category} para tienda ${store.id}:`,
            error,
          );
        }
      }
  ...
  async calculateSalesMetrics(storeId: string): Promise<void> {
    ...
    const yesterdaySales = await this.saleRepository.find({
      where: {
        store_id: storeId,
        sold_at: Between(yesterday, new Date(today.getTime() - 1)),
      },
    });
    ...
    const todayRevBs = filteredTodaySales.reduce(
      (sum, s) => sum + Number(s.totals?.total_bs || 0),
      0,
    );
    const todayRevUsd = filteredTodaySales.reduce(
      (sum, s) => sum + Number(s.totals?.total_usd || 0),
      0,
    );
    const yesterdayRevBs = filteredYesterdaySales.reduce(
      (sum, s) => sum + Number(s.totals?.total_bs || 0),
      0,
    );
    const yesterdayRevUsd = filteredYesterdaySales.reduce(
      (sum, s) => sum + Number(s.totals?.total_usd || 0),
      0,
    );
    ...
    const todayAvgBs =
      filteredTodaySales.length > 0
        ? todayRevBs / filteredTodaySales.length
        : 0;
    const todayAvgUsd =
      filteredTodaySales.length > 0
        ? todayRevUsd / filteredTodaySales.length
        : 0;
    const yesterdayAvgBs =
      filteredYesterdaySales.length > 0
        ? yesterdayRevBs / filteredYesterdaySales.length
        : 0;
    const yesterdayAvgUsd =
      filteredYesterdaySales.length > 0
        ? yesterdayRevUsd / filteredYesterdaySales.length
        : 0;
  ...
  async calculateInventoryMetrics(storeId: string): Promise<void> {
    ...
    const lowStock = products.filter(
      (p) =>
        ...
        Number(p.current_stock) <= p.low_stock_threshold,
    ).length;
    const outOfStock = products.filter(
      (p) => Number(p.current_stock) <= 0,
    ).length;
    const invValueBs = products.reduce(
      (sum, p) => sum + Number(p.current_stock) * Number(p.cost_bs || 0),
      0,
    );
    ...
    const expiringSoon = await this.lotRepository.count({
      where: {
        ...
        expiration_date: Between(now, thirtyDaysFromNow),
    ...
    const expired = await this.lotRepository.count({
      where: {
        product: { store_id: storeId },
        expiration_date: LessThanOrEqual(now),
        remaining_quantity: MoreThan(0),
      },
    });
  ...
  async calculateCustomerDebtMetrics(storeId: string): Promise<void> {
    ...
    const metricsToSave = [
      ...
      this.createMetric(
        ...
        now,
        now,
      ), // Estimado inicial hasta tener due_at
      this.createMetric(
        storeId,
        'debt',
  ...
  async calculateDailyAggregates(storeId: string): Promise<void> {
    ...
    const openShifts = await this.shiftRepository.find({
      where: { store_id: storeId, status: 'open' as any },
    });
    const cashOnHandBs = openShifts.reduce(
      (sum, s) =>
        sum +
        Number(s.opening_amount_bs || 0) +
        Number(s.expected_totals?.cash_bs || 0),
      0,
    );
  ...
  async calculateComparativeMetrics(
    ...
    const entityMetricType =
      dto.metric_type === DtoMetricType.SALES ? 'sales' : 'revenue';
