...
export class OversellAlertService {
      ...
      for (const stock of negativeStocks) {
        ...
        try {
          await this.notificationsService.createNotification(
            stock.warehouse_id,
            {
              // store_id is on warehouse? No, store_id is on warehouse entity usually. Let's check.
          ...
          this.logger.error(
            `Failed to send notification for oversell: ${error.message}`,
          );
        }
      }
    } catch (error) {
  ...
  async createOversellAlert(
    ...
    this.logger.warn(
      `Creating immediate oversell alert for event ${eventId}: ${warnings.join('; ')}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to create immediate oversell alert: ${error.message}`,
      );
    }
  }
}
